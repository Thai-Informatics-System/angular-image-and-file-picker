...
├── projects/
│   ├── angular-image-and-file-picker/   # Main library (Desktop side)
│   │   ├── src/
│   │   │   ├── lib/
│   │   │   │   ├── interfaces/           # TypeScript interfaces
│   │   │   │   ├── services/             # Core services
│   │   │   │   ├── angular-image-and-file-picker/  # Main component
│   │   │   │   ├── angular-qr-code-dialog/   # QR code dialog
│   │   │   │   ├── angular-file-viewer/      # File viewer
│   │   │   │   └── ...                   # Other components
│   │   │   └── public-api.ts             # Public exports
│   │   └── package.json
│   └── angular-mobile-upload/                # Mobile PWA (Mobile side)
│       ├── src/
│       │   ├── app/
│       │   │   ├── components/
...
### 1. Main Library: `angular-image-and-file-picker`
...
|-----------|----------|-------------|
| `AngularImageAndFilePickerComponent` | `angular-image-and-file-picker` | Main upload component with drag-drop, preview, and remote upload |
| `AngularFileViewerComponent` | `angular-file-viewer` | File preview dialog (PDF, Excel, images, video) |
| `AngularPreviewImageComponent` | `angular-preview-image` | Image preview with zoom/pan |
| `AngularQrCodeDialogComponent` | `angular-qr-code-dialog` | QR code dialog for mobile pairing |
| `AngularConfirmationDialogComponent` | `angular-confirmation-dialog` | Confirmation dialog |
| `AngularErrorDialogComponent` | `angular-error-dialog` | Error display dialog |

#### Key Services

| Service | Description |
|---------|-------------|
| `AngularRemoteUploadService` | Manages desktop-mobile pairing via WebSocket |
| `AngularHelperService` | Utility functions (snackbar, file helpers) |
...
- **`AngularRemoteUploadConfig`** - Remote upload feature configuration
- **`AngularSocketAdapter`** - Interface for host app's socket service integration
- **`OptionConfig`** - Component options (multiple, limit, compression, etc.)

### 2. Mobile Upload PWA: `angular-mobile-upload`

**Location:** `projects/angular-mobile-upload/`
...
│  │  ┌─────────────────────────┐         ┌─────────────────────────────────────────┐   │    │
│  │  │ Host's SocketService    │◄───────►│ AngularSocketAdapter                         │   │    │
│  │  │ (NgRx/Custom WebSocket) │         │ (implements interface, injected via DI) │   │    │
│  │  └─────────────────────────┘         └─────────────────┬───────────────────────┘   │    │
│  │                                                         │                             │    │
│  │                                                         ▼                             │    │
│  │  ┌──────────────────────────────────────────────────────────────────────────────┐   │    │
│  │  │ angular-image-and-file-picker Component                                 │   │    │
│  │  │                                                                               │   │    │
│  │  │  STATE: Not Connected                                                        │   │    │
│  │  │  ┌─────────────────────────────────────────────────────────────────┐        │   │    │
│  │  │  │ [Connect Mobile] button visible                                 │        │   │    │
│  │  │  │  ↓ User clicks                                                  │        │   │    │
│  │  │  │  Opens AngularQrCodeDialog                                          │        │   │    │
...
│  │                                                                                      │    │
│  │  ┌──────────────────────────────────────────────────────────────────────────────┐   │    │
│  │  │ AngularRemoteUploadService (Desktop-side WebSocket Manager)                      │   │    │
│  │  │                                                                               │   │    │
│  │  │  Key Observables:                                                            │   │    │
│  │  │  - remoteUpload$: BehaviorSubject<AngularRemoteUploadEvent>                     │   │    │
│  │  │    (emits when file is ACCEPTED by user)                                    │   │    │
│  │  │  - pendingFiles$: BehaviorSubject<AngularRemoteUploadEvent[]>                   │   │    │
...
│                              MOBILE BROWSER (PWA)                                         │
│  ┌──────────────────────────────────────────────────────────────────────────────────┐    │
│  │  angular-mobile-upload App (Angular PWA)                                              │    │
│  │                                                                                    │    │
...
#### Desktop (Library)
- `angular-remote-upload-paired-devices` - Array of paired mobile devices with session info
  ```json
  [{
  ...
  ```
- `angular-remote-upload-session-{mobileDeviceId}` - Active session data
  ```json
  {
...
#### Mobile (PWA)
- `angular-mobile-upload-session` - Current connection session
  ```json
  {
    "desktopDeviceId": "desktop_xyz",
    "mobileDeviceId": "fp_abc123",
  ...
  ```
- `angular-mobile-fingerprint` - Unique mobile device identifier (persisted across sessions)

### Pairing Flow (Token-Based Authentication)

1. **Desktop generates token:**
   - User clicks "Connect Mobile" button in library component
   - Component calls `AngularRemoteUploadService.openQrDialog()`
...
5. **Desktop acknowledges handshake:**
   - Desktop's `AngularSocketAdapter` receives INITIATED message via subscribed channel
   - `AngularRemoteUploadService` processes message
...
12. `AngularRemoteUploadService.handleUploadComplete()` adds file to `pendingFiles$` (NOT directly to `remoteUpload$`)
...
Host applications integrating the library **must implement** `AngularSocketAdapter` to connect the library to their WebSocket infrastructure:

```typescript
export interface AngularSocketAdapter {
  // ==================== Required Methods ====================
...
export class MySocketAdapterService implements AngularSocketAdapter {
...
```bash
# Build library
ng build angular-image-and-file-picker

# Build library in watch mode
ng build angular-image-and-file-picker --watch
...
```bash
# Build mobile app
ng build angular-mobile-upload

# Serve mobile app (development)
ng serve angular-mobile-upload --port 4201

# Build for production
ng build angular-mobile-upload --configuration=production
```
...
ng build angular-image-and-file-picker --configuration=production

# Navigate to dist folder
cd dist/angular-image-and-file-picker
...
### AngularRemoteUploadConfig Interface (NEW with onFileAccept)

```typescript
interface AngularRemoteUploadConfig {
  // ==================== Required ====================
  
  enabled: boolean;
  // Enable remote upload feature
  
  socketAdapter: AngularSocketAdapter;
  // Implementation of AngularSocketAdapter interface (provided by host app)
  ...
  onFileAccept?: (file: AngularRemoteUploadedFile) => void;
...
interface AngularRemoteUploadedFile {
...
export class MyFormComponent {
  ...
  remoteUploadConfig: AngularRemoteUploadConfig = {
    enabled: true,
  ...
  handleRemoteUpload(event: AngularRemoteUploadEvent) {
...
```
projects/angular-image-and-file-picker/src/lib/
├── interfaces/
│   ├── socket-adapter.interface.ts       # AngularSocketAdapter interface definition
│   ├── config.type.ts                    # UrlConfig, OptionConfig interfaces
│   ├── file-viewer-file-type.type.ts    # File type definitions
│   └── index.ts                          # Public exports
├── services/
│   ├── angular-remote-upload.service.ts      # Desktop-side WebSocket manager
│   │                                     # Key methods: sendFieldRequest, acceptPendingFile,
│   │                                     # rejectPendingFile, handleUploadComplete
│   └── angular-helper.service.ts             # Utility functions
├── angular-image-and-file-picker/
│   ├── angular-image-and-file-picker.component.ts
│   │                                     # Main upload component
│   │                                     # Handles pending files UI and accept/reject
│   ├── angular-image-and-file-picker.component.html
│   │                                     # Template with pending files section
│   └── angular-image-and-file-picker.component.css
│                                         # Styles including .pending-files-container
├── angular-qr-code-dialog/
│   ├── angular-qr-code-dialog.component.ts   # QR code display dialog
│   └── angular-qr-code-dialog.component.html
├── angular-file-viewer/                      # File preview components
├── angular-preview-image/                    # Image preview with zoom
└── public-api.ts                         # Library public exports
```

### Mobile PWA Files
```
projects/angular-mobile-upload/src/app/
├── components/
│   ├── upload/
...
```
docs/
└── angular-socket-adapter.service.ts         # Example AngularSocketAdapter implementation
...
├── Library project: angular-image-and-file-picker
└── Mobile PWA project: angular-mobile-upload

package.json                              # Root dependencies
projects/angular-image-and-file-picker/package.json  # Library dependencies
projects/angular-mobile-upload/package.json   # Mobile app dependencies
```
...
- NEW: onFileAccept callback in AngularRemoteUploadConfig
