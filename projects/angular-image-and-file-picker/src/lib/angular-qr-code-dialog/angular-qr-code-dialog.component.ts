...
import { AngularPairingSession, AngularRemoteUploadEvent } from '../interfaces/socket-adapter.interface';
...
export interface AngularQrCodeDialogData {
  title?: string;
  subtitle?: string;
  qrSize?: number;
...
@Component({
  selector: 'angular-qr-code-dialog',
  standalone: false,
  templateUrl: './angular-qr-code-dialog.component.html',
  styleUrls: ['./angular-qr-code-dialog.component.css']
})
export class AngularQrCodeDialogComponent implements OnInit, OnDestroy {
  qrData: string = '';
  expiresAt: number = 0;
  ...
  constructor(
    public dialogRef: MatDialogRef<AngularQrCodeDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: AngularQrCodeDialogData,
  ...
  private subscribeToEvents(): void {
    ...
    this.remoteUploadService.getErrors()
      .pipe(takeUntil(this.destroy$))
      .subscribe(error => {
        console.error('[AngularQrCodeDialog] Error:', error);
      });
  }
