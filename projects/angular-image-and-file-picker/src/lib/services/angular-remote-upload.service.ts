...
import {
  AngularSocketAdapter,
  AngularRemoteUploadConfig,
  AngularPairingSession,
  AngularRemoteUploadEvent,
  AngularRemoteUploadMessage,
  AngularRemoteUploadedFile
} from '../interfaces/socket-adapter.interface';

const DEFAULT_PAIRING_TTL = 24 * 60 * 60 * 1000; // 24 hours
const DEFAULT_STORAGE_KEY = 'angular-remote-upload-session';
...
export class AngularRemoteUploadService implements OnDestroy {
  private static readonly COMPONENT = 'AngularRemoteUploadService';
  private static readonly MOBILE_CONNECTION_KEY = 'angular-mobile-connection';
  // ponytail: read/migrate old localStorage key; remove after all clients refresh
  private static readonly LEGACY_MOBILE_CONNECTION_KEY = 'tis-mobile-connection';

  private destroy$ = new Subject<void>();
  private channelSubscription: Subscription | null = null;
  private healthCheckSubscription: Subscription | null = null;

  private config: AngularRemoteUploadConfig | null = null;
  private socketAdapter: AngularSocketAdapter | null = null;
  ...
  // State observables
  private pairingSession$ = new BehaviorSubject<AngularPairingSession | null>(null);
  ...
  private remoteUpload$ = new Subject<AngularRemoteUploadEvent>();
  private error$ = new Subject<string>();
  
  // Pending files from mobile - waiting for user to accept/reject
  private pendingFiles$ = new BehaviorSubject<AngularRemoteUploadEvent[]>([]);
   ...
   */
  async configure(config: AngularRemoteUploadConfig): Promise<void> {
    this.config = config;
    ...
    if (config.enabled && this.socketAdapter) {
      ...
      console.log(`[${AngularRemoteUploadService.COMPONENT}] Configured:`, {
   ...
   */
  getPairingSession(): Observable<AngularPairingSession | null> {
   ...
   */
  getRemoteUploads(): Observable<AngularRemoteUploadEvent> {
   ...
   */
  getPendingFiles(): Observable<AngularRemoteUploadEvent[]> {
   ...
   */
  getPendingFilesValue(): AngularRemoteUploadEvent[] {
   ...
   */
  acceptPendingFile(file: AngularRemoteUploadEvent): void {
    // Remove from pending
    ...
    console.log(`[${AngularRemoteUploadService.COMPONENT}] File accepted:`, file.file.fileName);
  }

  /**
   * Reject/delete a pending file
   */
  rejectPendingFile(file: AngularRemoteUploadEvent): void {
    const currentPending = this.pendingFiles$.value;
    this.pendingFiles$.next(currentPending.filter(f => f !== file));
    console.log(`[${AngularRemoteUploadService.COMPONENT}] File rejected:`, file.file.fileName);
  }
  ...
  async generateQrCode(): Promise<{ qrData: string; expiresAt: number }> {
    ...
    try {
      ...
      const session: AngularPairingSession = {
      ...
      console.log(`[${AngularRemoteUploadService.COMPONENT}] QR code generated, waiting for mobile...`);

      return { qrData, expiresAt };

    } catch (error: any) {
      const msg = `Failed to generate QR code: ${error.message}`;
      console.error(`[${AngularRemoteUploadService.COMPONENT}]`, msg);
      this.error$.next(msg);
      throw error;
    }
  }
  ...
  async sendFieldRequest(fieldInfo: {
    ...
    if (!this.isConnectedToMobile()) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Not connected to mobile, cannot send field request`);
      return;
    }

    const mobileDeviceId = this.mobileConnection$.value?.mobileDeviceId;
    const requestId = `field-${Date.now()}`;
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Sending field request to mobile:`, fieldInfo);
    ...
    try {
      ...
      console.log(`[${AngularRemoteUploadService.COMPONENT}] Field request sent:`, response);
    } catch (error: any) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Field request failed:`, error);
  ...
  async cancelFieldRequest(): Promise<void> {
    ...
    const mobileDeviceId = this.mobileConnection$.value?.mobileDeviceId;
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Canceling field request`);
    ...
    try {
      ...
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Cancel field request failed:`, error);
    }
  }
  ...
  private acceptMobileConnection(mobileDeviceId: string): void {
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Accepting mobile connection:`, mobileDeviceId);
    ...
    if (session) {
      const updatedSession: AngularPairingSession = {
        ...session,
        mobileDeviceId,
        status: 'connected',
        lastActivity: Date.now()
  ...
  async disconnect(): Promise<void> {
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Disconnecting from mobile...`);
    ...
    if (this.socketAdapter?.callApiViaSocket && mobileDeviceId) {
      try {
        ...
        console.log(`[${AngularRemoteUploadService.COMPONENT}] Disconnect API response:`, response);
      } catch (error: any) {
        console.warn(`[${AngularRemoteUploadService.COMPONENT}] Disconnect API call failed:`, error);
  ...
  private handleRemoteDisconnect(data: any): void {
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Mobile disconnected:`, data);
  ...
  startHealthCheck(): void {
    ...
    if (!mobileDeviceId || !this.deviceId) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Cannot start health check - missing device IDs`, {
        mobileDeviceId,
        desktopDeviceId: this.deviceId
      });
      return;
    ...
    if (!this.socketAdapter?.callApiViaSocket) {
      console.error(`[${AngularRemoteUploadService.COMPONENT}] Cannot start health check - socketAdapter.callApiViaSocket not available`);
      return;
    }

    console.log(`[${AngularRemoteUploadService.COMPONENT}] Starting health check (every ${HEALTH_CHECK_INTERVAL / 1000}s)`, {
      mobileDeviceId,
      desktopDeviceId: this.deviceId
    });
    ...
    this.checkDevicesOnline().catch(err => {
      console.error(`[${AngularRemoteUploadService.COMPONENT}] Initial health check failed:`, err);
    });
    ...
    this.healthCheckSubscription = interval(HEALTH_CHECK_INTERVAL)
      ...
      .subscribe(() => {
        this.checkDevicesOnline().catch(err => {
          console.error(`[${AngularRemoteUploadService.COMPONENT}] Periodic health check failed:`, err);
        });
      });
  }
  ...
  async checkDevicesOnline(): Promise<DevicesOnlineStatus | null> {
    ...
    if (!mobileDeviceId || !this.deviceId) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Cannot check devices - missing device IDs`, {
        mobileDeviceId,
        desktopDeviceId: this.deviceId
      });
      return null;
    ...
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Checking devices online status...`, {
      desktopDeviceId: this.deviceId,
      mobileDeviceId: mobileDeviceId
    });
    ...
    try {
      ...
      console.log(`[${AngularRemoteUploadService.COMPONENT}] Check devices response:`, response);
      ...
      console.log(`[${AngularRemoteUploadService.COMPONENT}] Devices status:`, {
      ...
      if (!status.desktop.isOnline && this.socketAdapter) {
        console.log(`[${AngularRemoteUploadService.COMPONENT}] Desktop detected as offline, sending immediate ping...`);
        this.callApiWithTimeout('ping', { timestamp: Date.now() }, 5000)
          .then(() => {
            console.log(`[${AngularRemoteUploadService.COMPONENT}] Immediate ping sent, rechecking status...`);
            // Recheck status after a short delay
            setTimeout(() => this.checkDevicesOnline(), 1000);
          })
          .catch((err) => {
            console.warn(`[${AngularRemoteUploadService.COMPONENT}] Immediate ping failed:`, err);
          });
      }

      return status;
    } catch (error: any) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Health check failed:`, error.message);
  ...
  private subscribeToChannel(channel: string): void {
    ...
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Subscribing to channel:`, channel);
    ...
    this.channelSubscription = this.socketAdapter.subscribeToChannel(channel)
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(`[${AngularRemoteUploadService.COMPONENT}] Channel error:`, error);
  ...
  private handleChannelMessage(message: any): void {
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Received:`, message);
    ...
    switch (type) {
      ...
      default:
        ...
        if (data.file || data.fileName || data.fileUrl) {
          ...
          console.log(`[${AngularRemoteUploadService.COMPONENT}] Unknown message type:`, type);
        }
    }
  }
  ...
  private handleMobileLinkEstablished(data: any): void {
    ...
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Mobile link established:`, {
      mobileDeviceId,
      mobileConnectionId,
      userId
    });
    ...
    if (mobileDeviceId) {
      ...
      console.log(`[${AngularRemoteUploadService.COMPONENT}] Connection established with mobile device:`, mobileDeviceId);
    }
  }
  ...
  private handleConnectionState(data: any): void {
    ...
    console.log(`[${AngularRemoteUploadService.COMPONENT}] Connection state:`, state, 'from:', mobileDeviceId);
  ...
  private handleUploadComplete(message: any): void {
    ...
    for (const file of files) {
      if (file && (file.s3Url || file.s3Path || file.resourceUrl)) {
        ...
        const normalizedFile: AngularRemoteUploadedFile = {
        ...
        const event: AngularRemoteUploadEvent = {
          file: normalizedFile,
        ...
        if (this.config?.autoAcceptFiles) {
          ...
          console.log(`[${AngularRemoteUploadService.COMPONENT}] File auto-accepted (showing in UI):`, normalizedFile.fileName);
          ...
          console.log(`[${AngularRemoteUploadService.COMPONENT}] File received (not auto-accepting, callback only):`, normalizedFile.fileName);
  ...
  private saveMobileConnection(info: MobileConnectionInfo): void {
    try {
      localStorage.setItem(
        AngularRemoteUploadService.MOBILE_CONNECTION_KEY, 
        JSON.stringify(info)
      );
    } catch (e) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Failed to save connection:`, e);
    }
  }
  ...
  private restoreMobileConnection(): void {
    try {
      const stored =
        localStorage.getItem(AngularRemoteUploadService.MOBILE_CONNECTION_KEY) ??
        localStorage.getItem(AngularRemoteUploadService.LEGACY_MOBILE_CONNECTION_KEY);
      if (stored) {
        ...
        if (Date.now() - info.lastActivity < DEFAULT_PAIRING_TTL) {
          ...
          this.saveMobileConnection(info);
          localStorage.removeItem(AngularRemoteUploadService.LEGACY_MOBILE_CONNECTION_KEY);
          console.log(`[${AngularRemoteUploadService.COMPONENT}] Restored mobile connection from localStorage:`, info.mobileDeviceId);
        } else {
          console.log(`[${AngularRemoteUploadService.COMPONENT}] Stored connection expired, clearing`);
          this.clearMobileConnection();
        }
      }
    } catch (e) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Failed to restore connection:`, e);
    }
  }
  ...
  private clearMobileConnection(): void {
    try {
      localStorage.removeItem(AngularRemoteUploadService.MOBILE_CONNECTION_KEY);
      localStorage.removeItem(AngularRemoteUploadService.LEGACY_MOBILE_CONNECTION_KEY);
    } catch (e) {
      console.warn(`[${AngularRemoteUploadService.COMPONENT}] Failed to clear connection:`, e);
    }
  }
